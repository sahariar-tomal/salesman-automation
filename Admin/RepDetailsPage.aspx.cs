...
public partial class RepDetailsPage : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // pass = dm.ReadRow(query1, conn)["pass"].ToString();
        ...
        string query3 = "select * from rep_schedule where rid= " + rid;
        areacode = dm.ReadRow(query3, conn)["areacode"].ToString();
        string anotherquery = "select * from schedule_8 where areacode=" + areacode;
        
        areaname = dm.ReadRow(anotherquery, conn)["areaname"].ToString();
        zone = dm.ReadRow(anotherquery, conn)["zone"].ToString();
        string query4 = "select * from bonusinfo where rid="+rid;
        rank = dm.ReadRow(query4, conn)["rank"].ToString();
        salary = dm.ReadRow(query4, conn)["salary"].ToString();
        totalorder = dm.ReadRow(query4, conn)["totalorder"].ToString();
        totaltarget = dm.ReadRow(query4, conn)["totaltarget"].ToString();
        totalbonus = dm.ReadRow(query4, conn)["totalbonus"].ToString();
